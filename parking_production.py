...
def delete_reservation(reservation_id):
    ...
    try:
        ...
        if affected > 0:
            print(f"🗑️  予約削除: ID {reservation_id}")
            return jsonify({'success': True, 'message': '予約を削除しました'})
        else:
...
<body>
    ...
    <script>
        ...
        async function loadReservations() {
            ...
            let html = '<table><tr><th>日付</th><th>時間帯</th><th>車両番号</th><th>氏名</th><th>金額</th><th>操作</th></tr>';
            data.reservations.forEach(r => {
                ...
                html += `<tr>
                    ...
                    <td>¥${r.amount}</td>
                    <td><button onclick="deleteReservation(${r.id}, '${r.date}', '${r.customer_name}')" style="background: #dc3545;">削除</button></td>
                </tr>`;
            });
            html += '</table>';
        ...
        async function deleteReservation(id, date, name) {
